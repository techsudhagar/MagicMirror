...
const CalendarFetcher = function (url, reloadInterval, excludedEvents, maximumEntries, maximumNumberOfDays, auth, includePastEvents) {
	...
	const calculateElapsedTime = function(diff_mins){

			if(diff_mins < 65 &&  diff_mins >= 60 ) {
				return true;

			} else if(diff_mins < 35 &&  diff_mins >= 30 ) {

				return true;

			} else if(diff_mins < 20 &&  diff_mins >= 15 ) {
				return true;

			} else if(diff_mins < 5 &&  diff_mins >= 0 ) {
				return true;	
			} 
